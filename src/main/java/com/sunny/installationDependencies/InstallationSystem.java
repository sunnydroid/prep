...
public class InstallationSystem {
	...
	public void addDependency(Component component, Set<Component> dependencies) {
		/* add component if it does not exist in the dependency list */
		if (!dependencyList.contains(component)) {
			dependencyList.add(component);
		}
		/* iterate through the dependencies, add them to the list if necessary
		 * and update them
		 */
		for (Component dependencyComponent : dependencies) {
			...
			/*
			 * update dependencyComponent to include component in it's usedby
			 * list
			 */
			dependencyComponent.addUsedByComponent(component);
			component.addUsesComponent(dependencyComponent);
			Logger.logComponentUsedBy(dependencyComponent);
		}
		
		Logger.logComponentUses(component);
	}

	public void installComponent(Component component) {
		if (installedList.contains(component)) {
			Logger.log("Component '" + component.getName()
					+ "' is aready installed");
			return;
		}
	...
	public void removeComponent(Component component) {
		...
		/*
		 * get list of dependencies and remove them if they aren't used =>
		 * recursive
		 */
		if (dependencyList.contains(component)) {
		...
		// TODO: iterate through the list of components that this component uses
		// and check if components in their
